...
function draw(data) {
  ...
  // 如果用户提供的color_palette 长度不为0 则使用它，否则使用d3.schemeCatetory20
  ...
  function getCurrentData(date) {
    ...
    //dataSort();
    currentData = currentData.slice(-max_number);
  ...
  function redraw() {
    ...
    if (auto_sort) {
      dateLabel
        .data(currentData)
        .transition()
        .duration(baseTime * interval_time* update_rate )
        .ease(d3.easeLinear)
    ...
    barEnter
      ...
      .transition("a")
      .delay(500 * interval_time* update_rate)
      .duration(2490 * interval_time* update_rate)
      .attr("y", 0)
    ...
    var barUpdate = bar
      .transition("2")
      .duration(2990 * interval_time* update_rate)
      .ease(d3.easeLinear);
    ...
    barExit.select(".text").attr("fill-opacity", 0);
    if (config.use_img) {
